  ...
  return (
    <div className="flex h-full bg-whatsapp-gray-light">
      <div className="w-[450px] flex flex-col border-r border-gray-200 bg-white">
        <div className="p-4 border-b border-gray-200 bg-white"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><div className="w-1 h-6 bg-whatsapp-green rounded-full"></div><h2 className="text-gray-900 font-medium">Chats ({sortedApplicants.length})</h2></div></div><div className="flex items-center gap-2"><Checkbox checked={isAllSelected} onCheckedChange={handleSelectAll} className="data-[state=checked]:bg-whatsapp-green data-[state=checked]:border-whatsapp-green" /><Button variant="ghost" size="sm" onClick={handleSelectAll} className="text-sm text-gray-600 hover:text-whatsapp-green p-0 h-auto">{isAllSelected ? 'Deselect All' : 'Select All'}</Button></div></div>
          ...
          {sortedApplicants.map((applicant) => (
            <div key={applicant.id} className={`group relative flex items-center p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors hover:z-20 focus-within:z-20 ${selectedChat === applicant.id ? 'bg-whatsapp-green-light border-l-4 border-l-whatsapp-green z-10' : ''}`}>
              ...
              <div className="flex items-center flex-1 cursor-pointer min-w-0" onClick={() => handleChatClick(applicant.id)}>
                <Avatar className="h-12 w-12 mr-3 flex-shrink-0"><AvatarFallback className="bg-whatsapp-green text-white">{applicant.phone.slice(-4)}</AvatarFallback></Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-whatsapp-gray flex-shrink-0 ml-2">{applicant.lastMessageTime}</span>
                  </div>
          ...
          <>
            <div className="flex-1 flex flex-col">
              <div className="p-4 border-b border-gray-200 bg-whatsapp-green text-white flex items-center justify-between">
                <div className="flex items-center gap-3"><Avatar className="h-10 w-10"><AvatarFallback className="bg-white text-whatsapp-green">{selectedApplicant.phone.slice(-4)}</AvatarFallback></Avatar><div><h3 className="font-medium text-white">{selectedApplicant.phone}</h3><p className="text-sm text-white/80">
